...
namespace System.Activities.Presentation.View
{
        ...
        internal class Quadrant
        {
            ...
            internal Quadrant Insert(T node, Circle bounds)
            {
                ...
                while (true)
                {
                    ...
                    Rect topLeft = new(toInsert.bounds.Left, toInsert.bounds.Top, w, h);
                    Rect topRight = new(toInsert.bounds.Left + w, toInsert.bounds.Top, w, h);
                    Rect bottomLeft = new(toInsert.bounds.Left, toInsert.bounds.Top + h, w, h);
                    Rect bottomRight = new(toInsert.bounds.Left + w, toInsert.bounds.Top + h, w, h);
    ...
    {
        private readonly QuadTree<object> quadTree;
        private static List<Circle> zones;

        public Vector3 Position { get; private set; }
        public float Radius { get; private set; }

        public RestrictedZone(float top, float left, float width, float height)
        {
            quadTree = new QuadTree<object>(top, left, width, height);
            zones = new List<Circle>();
            Position = Vector3.Zero;
        }

        public void AddZone(Vector2 position, float radius, Vector3 positionV3)
        {
            Circle zone = new(position, radius);
            zones.Add(zone);
            quadTree.Insert(null, zone);
            Position = positionV3;
            Radius = radius;
        }
        ...
        public bool IsInsideRestrictedZone(Vector2 position, float radius)
        {
            Circle testZone = new(position, radius);
            if (zones != null || zones.Count > 0)
            {
                foreach (var zone in zones)
                {
                    if (zone.IntersectsWith(testZone) && zone.isEmpty == false)
                    {
                        return true;
                    }
                }
        ...
        public IEnumerable<object> GetObjectsInsideRestrictedZones(Vector2 position, float radius)
        {
            Circle testZone = new(position, radius);
            List<object> objects = new();
            foreach (var zone in zones)
            {
                if (zone.IntersectsWith(testZone))
                {
                    objects.Add(null);
